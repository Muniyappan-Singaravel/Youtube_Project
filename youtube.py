...
def channel_basic():
    ...
    if selected_channel:
        mycursor.execute('''SELECT thumbnail, channel_name, playlist_id, channel_video,
                             channel_views, subscribers, channel_description FROM channel_details WHERE channel_name = %s''',
                         (selected_channel,))
...
# function to delete the channel details if needed
def delete_channel():

    channel_id = st.text_input("Enter Channel ID:")

    if channel_id:
        mycursor.execute("SELECT channel_id FROM channel_details WHERE channel_id = %s", (channel_id,))
        result = mycursor.fetchone()

        if result:
            query1 = '''DELETE  FROM comment_details 
                    WHERE comment_video_id IN 
                    (SELECT video_id FROM video_details  
                    WHERE channel_id = %s )'''

            query2 = '''DELETE FROM video_details  WHERE channel_id = %s'''

            query3 = '''DELETE FROM channel_details  WHERE channel_id = %s'''
            
            with st.spinner("Processing..."):
                mycursor.execute(query1, (channel_id,))
                mycursor.execute(query2, (channel_id,))
                mycursor.execute(query3, (channel_id,))
                mydb.commit()
            st.success("All Datas Related To This Channel ID Has Been Deleted!")
...
selected_page = st.sidebar.radio("Pages", ['Home', 'Channel Detail', 'Add Channel', "Questions", "Delete Channel"])
...
if selected_page == "Home":
    ...
    st.write("Click the below button to transfer data to the database!")
...
elif selected_page == "Delete Channel":
    st.subheader(":violet[Delete Channel]")
    st.markdown(":blue-background[_Remove All Datas Of Channel If Not Needed!_]")
    delete_channel()
